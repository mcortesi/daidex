...
import { toTokenDecimals, toWei, fromTokenDecimals } from "./units";
      ...
      return {
        message: () =>
          `expected ${received.toString(
            10
          )} not to be equal to ${argument.toString(10)}`,
        pass: true
      };
    } else {
      return {
        message: () =>
          `expected ${received.toString(10)} to be equal to ${argument.toString(
            10
          )}`,
        pass: false
      };
...
function createOrder(
  opts: Pick<Order, "volume" | "volumeEth" | "fee" | "remaining">
): Order {
...
describe("obmath", () => {
  ...
  describe("getMinVolume()", () => {
    test("simple case", () => {
      ...
      expect(minVolume).toEqualBN(
        toTokenDecimals("0.0059820538", TokenDecimals)
      );
    });
  });
  ...
  describe("getTransactionFor()", () => {
    test("case: 1 order - partial completion", () => {
      ...
      const tx = obmath.getTransactionFor(
        orders,
        1,
        toTokenDecimals(20, TokenDecimals)
      );
      ...
      expect(tx.requiredVolumeEth).toEqualBN(requiredVolumeEth);
      expect(tx.currentVolumeEth).toEqualBN(requiredVolumeEth);

      // this selection is valid until we reach 30 tokens (max volume)
      expect(tx.maxAvailableVolume).toEqualBN(
        toTokenDecimals(30, TokenDecimals)
      );
    ...
    test("case: 1 order - partial completion - order is not full", () => {
      ...
      const tx = obmath.getTransactionFor(
        orders,
        1,
        toTokenDecimals(10, TokenDecimals)
      );
    ...
    test("case: 2 orders - partial completion", () => {
      ...
      const tx = obmath.getTransactionFor(
        orders,
        2,
        toTokenDecimals(40, TokenDecimals)
      );

      expect(tx.orders).toHaveLength(2);
      expect(tx.orders.map(o => o.id)).toEqual(["order-1", "order-2"]);

      const requiredVolumeEth = orders[0].volumeEth.add(
        orders[1].volumeEth.divn(2)
      );
      expect(tx.requiredVolumeEth).toEqualBN(requiredVolumeEth);
    });
  ...
  describe("StdTransactionInfo", () => {
    test("consume ALL extraVolume", () => {
      const tx = new obmath.StdTransactionInfo({
        requiredVolume: new BN(1000000),
        requiredVolumeEth: new BN(200000),
      ...
      expect(tx.currentVolume).toEqualBN(tx.requiredVolume);
      expect(tx.currentVolumeEth).toEqualBN(tx.requiredVolumeEth);

      // Consume Total extra space
      tx.changeVolume(tx.currentVolume.add(tx.extraVolume));

      expect(tx.currentVolume).toEqualBN(tx.requiredVolume.add(tx.extraVolume));
      expect(tx.currentVolumeEth).toEqualBN(
        tx.requiredVolumeEth.add(tx.extraVolumeEth)
      );
    });
    test("NO extraVolume", () => {
      const tx = new obmath.StdTransactionInfo({
      ...
      expect(tx.currentVolume).toEqualBN(tx.requiredVolume);
      expect(tx.currentVolumeEth).toEqualBN(tx.requiredVolumeEth);
      expect(tx.maxAvailableVolume).toEqualBN(tx.requiredVolume);
      expect(tx.maxAvailableVolumeEth).toEqualBN(tx.requiredVolumeEth);
    });
  });
});
